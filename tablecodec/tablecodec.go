...
const (
	estimatedRowKeyLen    = 16
	estimatedIdxPrefixLen = 12
)

// EncodeRowKey encodes the table id and record handle into a kv.Key
func EncodeRowKey(tableID int64, encodedHandle []byte) kv.Key {
	buf := make([]byte, 0, estimatedRowKeyLen)
...
func DecodeRecordKey(key kv.Key) (tableID int64, handle int64, err error) {
	...
	key, handle, err = codec.DecodeComparableVarint(key)
	if err != nil {
		return 0, 0, errors.Trace(err)
	}
	return
...
func EncodeRow(row []types.Datum, colIDs []int64) ([]byte, error) {
	...
	for i, c := range row {
		id := colIDs[i]
		b = codec.EncodeComparableVarint(b, id)
		fc, err := flatten(c)
		if err != nil {
			return nil, errors.Trace(err)
		}
		b, err = codec.EncodeValue(b, fc)
		if err != nil {
			return nil, errors.Trace(err)
		}
	}
	if len(b) == 0 {
		...
		// So encode a byte 0 to represent nil.
		b = []byte{0}
	}
	return b, nil
}
...
func DecodeRow(b []byte, cols map[int64]*types.FieldType) (map[int64]types.Datum, error) {
	...
	for len(b) > 0 {
		// Get col id.
		b, cid, err = codec.DecodeComparableVarint(b)
		if err != nil {
			return nil, errors.Trace(err)
		}
		// Get col value.
...
func CutRow(data []byte, cols map[int64]*types.FieldType) (map[int64][]byte, error) {
	...
	for len(data) > 0 && cnt < len(cols) {
		// Get col id.
		data, cid, err = codec.DecodeComparableVarint(data)
		if err != nil {
			return nil, errors.Trace(err)
		}
		// Get col value.
...
func EncodeIndexSeekKey(tableID int64, idxID int64, encodedValue []byte) kv.Key {
	key := make([]byte, 0, estimatedIdxPrefixLen+len(encodedValue))
...
func DecodeIndexKey(key kv.Key) ([]types.Datum, error) {
	b := indexKeyWithoutPrefix(key)
	return codec.Decode(b, 1)
}

func indexKeyWithoutPrefix(key kv.Key) []byte {
	tbIDlen := codec.ComparableVarintLen(key[1])
	idxIDLen := codec.ComparableVarintLen(key[1+tbIDlen+1])
	return key[1+tbIDlen+1+idxIDLen:]
}
...
func EncodeTableIndexPrefix(tableID, idxID int64) kv.Key {
	key := make([]byte, 0, estimatedIdxPrefixLen)
	key = appendTableIndexPrefix(key, tableID)
	key = codec.EncodeComparableVarint(key, idxID)
	return key
}
...
func TruncateToRowKeyLen(key kv.Key) kv.Key {
	...
	tbIDlen := codec.ComparableVarintLen(key[1])
	if len(key) < tbIDlen+2 {
		return key
	}
	rowIDLen := codec.ComparableVarintLen(key[tbIDlen+2])
	rowKeyLen := tbIDlen + 2 + rowIDLen
	if len(key) < rowKeyLen {
		return key[:rowKeyLen]
	}
	return key
}
