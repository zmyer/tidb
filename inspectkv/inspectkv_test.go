...
func (s *testSuite) testIndex(c *C, tb table.Table, idx table.Index) {
	...
	c.Assert(err, IsNil)
	key := tablecodec.EncodeRowKey(tb.Meta().ID, codec.EncodeComparableVarint(nil, 4))
	...
	c.Assert(err, IsNil)
	key = tablecodec.EncodeRowKey(tb.Meta().ID, codec.EncodeComparableVarint(nil, 3))
	...
	key = tablecodec.EncodeRowKey(tb.Meta().ID, codec.EncodeComparableVarint(nil, 3))
	txn.Delete(key)
	key = tablecodec.EncodeRowKey(tb.Meta().ID, codec.EncodeComparableVarint(nil, 5))
